...
async function loadLedgerState(magicString) {
  ...
  await execShellCommand(cardanoCLI + ' shelley query ledger-state --allegra-era --cardano-mode ' + magicString + ' > ' + process.cwd() + '/ledgerstate.json ')
...
async function calculateLeaderLogs() {
  ...
  const protocolParameters    = await callCLIForJSON(cardanoCLI + ' shelley query protocol-parameters --allegra-era --cardano-mode ' + magicString)
